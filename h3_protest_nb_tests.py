"""H3: Discourse -> next-week PROTEST events (count)

Same setup as H2, but outcome is protest_next_week.

Outcome:
- protest_next_week  (count, next-week)
...
def main():
    ...
    required = {"dyad", "year", "week", "num_docs", "protest_next_week"}
    ...
    alpha = float(args.alpha) if args.alpha is not None else estimate_alpha_nb(df["protest_next_week"])
    ...
    for f in DISCOURSE_FEATURES:
        x = scaled_map[f]
        formula = f"protest_next_week ~ {x} + num_docs + C(dyad) + C(year) + {week_term}"
    ...
    out_csv = out_dir / "h3_protest_feature_tests.csv"
    ...
    meta_json = out_dir / "h3_protest_meta.json"
    meta_json.write_text(json.dumps(meta, indent=2), encoding="utf-8")

    print("Saved:", out_csv)
    print("Saved:", meta_json)
    print("\n=== H3 protest (feature tests) ===")
